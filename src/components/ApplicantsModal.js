...
import { 
  ...
  updateVacancyCounts,
  markApplicationAsViewed // <--- ✅ ADDED IMPORT
...
export default function ApplicantsModal({ vacancy, onClose }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // --- ✅ NEW: MARK PENDING APPLICATIONS AS VIEWED ---
        interests.forEach(async (app) => {
  ...
  const handleStatusChange = async (appId, newStatus, workerId, currentStatus) => {
    ...
    setApplications(prev =>
      prev.map(app =>
        app.id === appId
          ? { ...app, status: newStatus, workerPhone: phone }
          : app
      )
    );
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="border rounded-lg p-4 flex flex-col md:flex-row justify-between gap-4 bg-white shadow-sm"
                >
                  <div>
                    <h4
                    ...
                    >
                      ...
                      <span className="text-sm font-normal text-gray-500">
                        ({app.workerProfile?.age} yrs)
                      </span>
                    </h4>

                    <p className="text-sm text-gray-600">
                      📍 {app.workerProfile?.district}, {app.workerProfile?.state}
                    </p>
                    <p className="text-sm text-gray-600">
                      🛠 {app.workerProfile?.skills?.join(", ")}
                    </p>
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() =>
                                openWhatsAppChat(
                                  app.workerPhone,
                                  `Hello, we have accepted your application for ${vacancy.jobTitle}.`
                                )
                              }
                            ...
                            <a
                              ...
                              className="bg-blue-500 text-white px-3 py-1.5 rounded font-bold hover:bg-blue-600"
                            >
                              📞 Call
                            </a>
                          </div>
                  ...
                  <div className="flex flex-col gap-2 justify-center min-w-[140px]">
                    {app.status === "pending" && (
                      <>
                        <button
                          onClick={() =>
                            handleStatusChange(app.id, "accepted", app.workerId, app.status)
                          }
                        ...
                        <button
                          onClick={() =>
                            handleStatusChange(app.id, "rejected", app.workerId, app.status)
                          }
                          className="bg-red-50 text-red-600 border border-red-200 px-3 py-1.5 rounded text-sm hover:bg-red-100"
                        >
                          ❌ Reject
                        </button>
                      </>
                    ...
                    {app.status === "accepted" && (
                      <>
                        <span className="text-center text-green-600 font-bold text-sm">
                          Accepted
                        </span>
                        <button
                          onClick={() =>
                            handleRateWorker(app.workerId, app.workerProfile?.name)
                          }
                          className="text-xs bg-yellow-50 text-yellow-700 border border-yellow-200 px-2 py-1 rounded"
                        >
                          ⭐ Rate Worker
                        </button>
                        <button
